/** Renders the drop-cap fixture to a real DOCX so WORD ITSELF can lay it out (CTO-required). */
...
const now = new Date();
const blocks: Paragraph[] = Array.from({ length: 12 }, (_, i) => ({
  type: 'paragraph' as const, id: `p${i}`, text: TEXT, inlines: [], dropCap: true,
}));
...
writeFileSync(join(OUT, 'with-dropcaps.docx'), result.output);
console.log('wrote with-dropcaps.docx');
