...
3. **§7 last bullet — widows/orphans (was "pending further discussion")**: headings/titles hard gate **0** — enforced by construction (title keep-with-next + `staysWithNext`) and measured 0; body-text widows/orphans hard gate **0 by the min-2-lines rule** (Phase B splits never strand fewer than 2 lines at either end), observed 0. The "body advisory" hedge can be retired: the mechanism guarantees it. **Amended 2026-07-22 (MINI_DR_BLOCKLESS_TITLES):** the "enforced by construction" claim for headings was measured FALSE on one real page — a blockless titled section's uncharged title stranded an 18pt heading at the bottom of Modern p.40 (`TYPOGRAPHY_QUALITY_SCOPE.md` §1); the charge is fixed and the gate is now **renderer-enforced by a named counter** (`RenderMetrics.unplannedTitleBreaks`, pinned 0 across all themes on the real corpus in `blocklessTitleParity.test.ts`) — a test, no longer a construction argument. The body-line half of this value was re-verified true on 494 real pages the same day.
...
### 10.4 Second theme — Modern calibration (2026-07-21, `SECOND_THEME.md`)

The Modern theme (sans headings + Georgia body + a **visible** Prussian accent, slightly tighter heading spacing) gets its **own** parity numbers — never reusing Classic's — measured on `faith-alone-styled.docx` and locked in `modernTheme.test.ts`:

| Theme | Layout | Real pages | Reconciliations |
|---|---|---|---|
| Classic | letter | 90 | 2 |
| Modern | letter | 90 | 2 |
| Classic | kdp-6x9 | 159 | 2 |
| Modern | kdp-6x9 | **158** | 2 |

Modern's R2 contract holds by the same mechanism (measured real-face line heights): `unplannedPageBreaks = 2`, within the locked bound. It tracks Classic closely; the one-page difference at kdp-6x9 is the tighter heading spacing (12 vs 16pt) — Modern's own number, not drift. Classic's numbers and visual baseline are unchanged (byte-stable; verified — Classic's export never carries Modern's accent). The full six-layout Modern table can be regenerated by `quality-bar-calibration-spike.ts`; the two above are the R2-locked anchors. **This advances `RECALIBRATE_PAGE_RATIO_TOLERANCE`:** the corpus now carries a second *theme* dimension, not only a second fixture — a point for whoever re-derives the ±8% tolerance. *(The table above is the 2026-07-21 record; the LIVE lock is `modernTheme.test.ts`, consciously re-locked since — e.g. letter 90 → 88 at the subtitle-spacing chantier.)*

### 10.5 Third theme — Novel calibration (2026-07-22, `THIRD_THEME_NOVEL.md`)

The Novel theme (Gelasio throughout, generous title rhythm 22/10, `chapterTitle` running head,
warm accent) is **the first theme born with drop caps LIT** (`chapterOpening` @2.5): its parity
numbers include the ornament's priced height on every chapter opening — 17 drop caps fire on
faith-alone. Measured 2026-07-22 and locked in `novelTheme.test.ts`:

| Theme | Layout | Real pages | Reconciliations | Drop caps fired |
|---|---|---|---|---|
| Novel | letter | 90 | **1** | 17 |
| Novel | kdp-6x9 | 161 | **1** | 17 |

Charged == consumed holds WITH the ornament live (`degradedDropCaps = 0`, reconciliations within
the residual bound on both anchors) — the drop-cap capability's pricing instruments carrying a
real shipped theme for the first time. Classic's and Modern's numbers are untouched (their
`scope:'none'` locks re-ran green); the third *theme* dimension joins the calibration space
(same `RECALIBRATE_PAGE_RATIO_TOLERANCE` note as §10.4 — the FIXTURE condition stays 2 of 3).
