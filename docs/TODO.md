...
- **`DROPCAP_TEXT_OVERLAP`** — **real rendering defect, PDF only, latent** (found 2026-07-21, CTO-classified). Named independently of the Book Presentation chantier: this is a text-layout bug that the theme work merely uncovered, not a theming question.
  ...
  **Latent — no book shipped to date suffers from it.** Nothing populates `Block.dropCap` anywhere in the pipeline (measured: 0 drop caps across 2,152 real paragraphs, `dropcap-feasibility-spike.ts`), so the defect has never had a producer. It is live code on `main` with no trigger. **This is a reassuring fact to keep visible, not one to minimise** — and it is precisely why the drop-cap theme capability is frozen: building the positional trigger would hand a freshly documented latent bug its first real user.
  ...
  **Pricing is back in scope — the earlier removal is annulled.** The 2026-07-21 decision removing `estimateBlockHeight` pricing for `dropCap` rested on a measured height invariance (25 → 25 pages, 0 → 0 reconciliations) that Word's own layout later showed to be **the signature of this very bug**, not evidence of no cost: Word goes 3 → 4 pages on the same 61 lines and 397 words, because it grows line boxes where PDFKit overlaps them. Annulled formally, with the full reasoning, in `docs/DECISIONS.md` — *"R2 verifies that charged equals consumed, not that what is consumed is correct"*. Pricing is the **central** subject of the fix's review, not a compliance section.

  **Fixing it needs its own mini Design Review** — it changes text layout in `PDFRenderer`, not a theme value: what changes, how R2 stays guaranteed once horizontal indentation enters the picture, and the risk if that is wrong. **Blocks the drop-cap theme capability entirely** (trigger, theme and renderer wiring alike) until fixed and verified.
...
- **`DROPCAP_PARAGRAPH_ATOMICITY`** — accepted disclosed tech debt (CTO decision 2026-07-21, same status as its cousin `LIST_SPLITTING_ACROSS_PAGES` above). A paragraph carrying a drop cap is excluded from Phase B line-splitting (`LayoutEngine.ts:222`, deliberate — a drop cap's first lines are typographically special), so a drop-cap paragraph that does not fit the remaining page space cannot split: it moves whole and, if the page cannot hold it, produces a reconciliation page. **Not corrected now, and correcting it is not planned.**
  ...
  **The justification is STRUCTURAL, not about the drop cap's height** (reformulated 2026-07-21 after measurement, CTO-directed — the first wording was wrong and is corrected rather than quietly replaced). The exclusion makes the *whole paragraph* atomic regardless of what the glyph costs; a long drop-cap paragraph overflows for exactly the reason a long list item does — **it cannot be split, full stop**. It is NOT "the drop cap's height can cause an overflow": measured end-to-end (`backend/spikes/dropcap-height-spike.ts`, 120 drop-cap paragraphs vs 120 plain), a drop cap consumes **zero** extra height — model 25 → 25 pages, real 25 → 25, reconciliations 0 → 0. There is consequently **no pricing gap and nothing to charge**; adding a pricing term would over-charge and produce under-full pages (CTO removed it from scope on that measurement).

  **The CTO's condition: the debt may stay uncorrected but may not stay unmeasured.** The drop-cap mini Design Review must ship a test proving the property — a drop-cap paragraph too long for the page remainder yields a *measured reconciliation*, never a silent overflow — the letter of ADR-0051: loud, not silent. Candidate for a future splitting review alongside its cousin, not a hidden gap.
...
**Sprint 18+ (Editorial AI Engine) — ⏸️ DEFERRED, review preserved not withdrawn.** `docs/architecture/diagrams/EDITORIAL_AI_ENGINE.md` keeps its round-1 draft intact: 6 open questions, real code evidence (zero AI references, zero outbound HTTP, no secret management), and one unresolved tension between `VISION.md`'s accept/reject workflow and Sprint 7 Decision 2's stateless backend. A future session should re-verify that evidence before trusting it — Sprints 11/12/16 are expected to change the answers to its Questions 3 and 6.
