...
**Next: Commit 4 (`feat(frontend): API client typed against packages/shared-types`)** — `lib/api-client.ts`, no UI yet; this is the first commit expected to produce a `frontend/`-side Visible Increment entry once it's paired with the following commit that actually renders something (per the Kickoff's own mapping, the dev-server-boots increment lands around Commit 4/5).
