...
**Last Updated:** July 18, 2026 (Sprint 7 "First Demonstrable Product" released as `v0.8.0-alpha`, PR #12 merged; Sprint 8 not yet started)
...
- [~] **Publishing Engine** (see `PLATFORM_ARCHITECTURE_ROADMAP.md` §2.5) — KDP/Kobo/Apple Books/Google Play Books packaging, sits after the `Renderer` port; owns `PostRenderValidation` per `VALIDATION_ENGINE.md`'s Sprint 5 scope decision. **Confirmed as Sprint 8's target (2026-07-18, CTO direction). Level 2 Design Review drafted, round 1: `docs/architecture/diagrams/PUBLISHING_ENGINE.md`** — 5 open questions posed for CTO review (architectural framing vs. `VISION.md`'s original model, first platform + spike need, `PostRenderValidation` ownership, HTTP surface shape, explicit scope-narrowing against real automated KDP submission). Not approved yet, no branch, no code. Reconciles the overlap with the "Kindle / Kobo / Lulu / IngramSpark / Amazon KDP export targets" Backlog item below, as that item itself already anticipated.
