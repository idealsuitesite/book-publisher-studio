...
**Sprint 7 scope — Design Review drafted (2026-07-18), awaiting CTO approval, not yet started.** CTO proposal (2026-07-17, post-Sprint-6): make Sprint 7 the project's first interactive demo (UI import → structure view → format change via `LayoutSelector` → preview → PDF/DOCX/EPUB export) instead of the next backend-only engine — substantively `v0.8.0-alpha` "Premium UI/UX" moved ahead of Plugin System/Editorial AI Engine/Publishing Engine. Design Review draft: `docs/architecture/diagrams/PREMIUM_UI_UX.md` — real evidence gathered (exact backend API shapes, an already-existing untouched `frontend/` Next.js 16 scaffold discovered during evidence-gathering, no persistence/no live-preview-endpoint confirmed), 5 open decisions flagged for explicit CTO resolution (what "preview" means for Sprint 7, how a stateless backend gets re-supplied the manuscript across requests, how much of VISION.md's fuller UI ambition ships now vs. later, shared-types-or-not between `frontend/`/`backend/`, a new theme/layout-discovery endpoint or not), draft commit plan and acceptance criteria. **Status: DRAFT, round 1 — not approved.** No `docs/VERSIONS.md` renumbering, no branch, no code until this review is explicitly approved (`docs/DESIGN_REVIEW_PROCESS.md`'s own gate). The professional editorial fixture library (novels/technical/academic/magazine/bible/children/poetry/cookbook, see Backlog below) remains deferred until this scope is fixed.
