...
**Next: Commit 3 (`feat(backend): DTOs re-exported from packages/shared-types`)** — `BookDTO`, `ImportReportDTO`, `ValidationIssueDTO`, `QualityScoreDTO` move to `packages/shared-types`; no behavior change, existing test suite must stay green.
