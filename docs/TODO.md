...
**Last Updated:** July 17, 2026 (Sprint 4 released as `v0.5.0-alpha`; Sprint 5 Validation Engine Design Review approved, implementation not yet started)
...
**Sprint 5 (Validation Engine)** — Design Review ✅ APPROVED (2026-07-17), two levels: `docs/architecture/diagrams/PLATFORM_ARCHITECTURE_ROADMAP.md` (Level 1 — 5-engine platform map) and `docs/architecture/diagrams/VALIDATION_ENGINE.md` (Level 2 — Sprint 5's full design, `ValidationEngine` orchestrating a `RuleRegistry` of independent rules). ADR-0027 (Validation Engine is read-only) written. **No code yet, no branch yet** — implementation starts only once the CTO gives explicit go-ahead to branch, per the CTO's own framing that this finalization pass happens before any implementation.
...
- [x] ~~`ValidatorEngine`~~ — now **Validation Engine**, Sprint 5, Design Review approved (see IN PROGRESS above). No longer a vague backlog item.
...
- [ ] **Editorial AI Engine** (see `docs/VISION.md`'s dedicated section, and `PLATFORM_ARCHITECTURE_ROADMAP.md` §2.2 for its now-fixed dependency on Validation Engine's output) — independent module, own pipeline stage between Normalizer and Theme Engine, entirely separate from rendering so the rendering pipeline stays deterministic. Humanization, grammar/style correction, accept/reject suggestions, readability analysis, manuscript consistency checks, AI writing assistant. Realistically its own Sprint 6/7. **Not scoped in detail, no code/ADR/Level-2-Design-Review yet** — explicitly deferred until Validation Engine (Sprint 5) is fully merged and verified, since Editorial AI Engine now has a real, ordered dependency on it.
- [ ] **Professional Layout Engine** (see `PLATFORM_ARCHITECTURE_ROADMAP.md` §2.4) — likely `LayoutEngine` extended, not a new class; not yet confirmed. No Design Review, no Sprint assignment.
...
**Sprint 5+ priority ordering — RESOLVED (2026-07-17) by the actual Sprint 5 Design Review**, superseding both of the two competing proposals recorded here during Sprint 4: **Validation Engine is Sprint 5** (matches the original CTO priority order's #2 slot, "ValidatorEngine," now formally named and designed — `docs/architecture/diagrams/VALIDATION_ENGINE.md`). Editorial AI Engine, Plugin System, Professional Layout Engine, and Publishing Engine are all mapped in `docs/architecture/diagrams/PLATFORM_ARCHITECTURE_ROADMAP.md` §2 with real dependencies fixed (notably: Editorial AI Engine now depends on Validation Engine's output, so it cannot ship before Validation Engine regardless of which competing proposal's ordering is preferred) but **no Sprint 6+ assignment or relative ordering among them decided yet** — that's each engine's own future Design Review to resolve, one at a time, same discipline as this one.
...
**Sprint 5 (Validation Engine) Design Review — ✅ APPROVED (2026-07-17)**, two levels (`docs/architecture/diagrams/PLATFORM_ARCHITECTURE_ROADMAP.md` + `docs/architecture/diagrams/VALIDATION_ENGINE.md`), two review rounds. Final architecture: `ValidationEngine` orchestrates a `RuleRegistry` of independent, pure `ValidationRule`s (`StructuralRule`, `MetadataRule`, `HeadingRule`, `MissingRequiredStyleRule`, `TypographyRule`, `ImageRule`, `HyperlinkRule`, `ComplianceRule`); `validate(context: ValidationContext)` replaces the originally-proposed `validate(book, paginated?)` to stabilize the public API against future per-platform rule variants; new `ValidationSeverity` (`ERROR`/`WARNING`/`INFO`/`SUGGESTION`) generalizes today's binary error/warning split, with `ValidationReport.errors`/`.warnings` kept as backward-compatible derived views. Round-1 open questions resolved: "missing styles" doesn't exist as a concept and was replaced with the narrower `MissingRequiredStyleRule`; KDP/EPUB compliance is `PreRenderValidation`-only this sprint (`PostRenderValidation` deferred to the future Publishing Engine); hyperlink checks are syntactic-only, no network I/O. **ADR-0027 (Validation Engine is read-only) written.** **No implementation yet — awaiting explicit go-ahead to branch.**
