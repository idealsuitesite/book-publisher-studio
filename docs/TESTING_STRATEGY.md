...
**`PARITY_LOCK_UNCHANGED` — a parity lock may legitimately NOT change, and that must be verified, not assumed (precedent set 2026-07-25, AUTHOR_EXPERIENCE M3-C8).** The RENDER_DRIFT/ADR-0051 discipline is usually stated for the case where output changes: *a lock that changes silently is a lie; a lock that changes with its reason recorded is the system working.* Its twin is the case where output does NOT change even though the render code did. C8 moved the scene-break separator into the theme (Classic's went from `* * *` to a drawn rule), a renderer touch that *looks* like it should move the faith-alone page-count lock — but `LayoutEngine` charges a divider exactly one `line` regardless of style, and every rendered style consumes exactly one line, so pagination is untouched and only the glyph differs (which the page-count lock does not measure). **The standing formulation: a lock that legitimately doesn't change is verified, not assumed** — you reason WHY it should hold from the model (here: charged-height is style-independent), and then you run it and confirm the number is identical (237/235 held, no re-lock). A renderer touch that leaves a parity lock unmoved is only trustworthy when both halves are done: the reason and the measurement. Skipping the run because "it shouldn't change" is the same failure as re-locking silently — assuming instead of measuring.
