...
8. **Cadence d'approbation** — l'autonomie intra-chantier est acquise une fois la porte franchie. Les approbations vivent aux portes (bootstrap, DR, cadrage→verdict, arrêts-goût, fusion, push) ; entre elles, la séquence approuvée se construit sans redemander, avec trois arrêts d'office : prémisse contredite par la mesure, défaut de fidélité actif, sortie de périmètre. → ADR-0054, `docs/DECISIONS.md`.
