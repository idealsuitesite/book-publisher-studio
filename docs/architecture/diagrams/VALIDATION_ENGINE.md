...
**Decision 2 — KDP/EPUB compliance: pre-render only, Sprint 5.** Two explicit categories going forward, matching the CTO's own naming:
   ...
   - **`PostRenderValidation`** (future, explicitly not this sprint, likely belongs to `Publishing Engine` per `PLATFORM_ARCHITECTURE_ROADMAP.md` since that's the engine that owns platform-specific packaging) — rendered-output correctness: real page count, embedded-font validity, EPUB structural validity. Named here so it isn't lost, not designed.
     - **✅ Closed (Sprint 8, 2026-07-18):** built in Publishing Engine exactly as anticipated — `SubmissionValidator` orchestrating a `PostRenderValidationRule` family, mirroring this engine's own `ValidationEngine`/`RuleRegistry` shape (`PUBLISHING_ENGINE.md` Decision 3). **`ValidationEngine` itself was not modified** — this stayed a separate rule family in a separate engine, as this decision intended. Of the three examples named above, Sprint 8 shipped rules for required metadata, page-count bounds, cover presence, and accepted interior format; **real page count is not actually checkable yet** (the pagination data never reaches the rule — ADR-0038, deferred), and embedded-font/EPUB-structural validity were not built.
