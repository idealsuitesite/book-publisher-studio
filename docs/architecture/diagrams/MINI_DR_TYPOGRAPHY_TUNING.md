...
**Status:** ✅ CTO-APPROVED (2026-07-22 — §2.1's offset interpretation validated explicitly: "meilleur que ce que j'avais demandé", the future-theme identity argument) then **BUILT AND MERGED** in 4 commits as planned. Backend **734/734**, frontend **206/206**, tsc + eslint clean both sides. **The CTO's two closure attention points, both delivered:** the cache key tested BOTH directions on one book (preset change → MISS; accent change over the overridden book → HIT; clear → legitimate HIT on the cached no-override geometry) and the real-fixture lock (faith-alone at Large + sans: >+25% band, unplanned ≤ residual class, tri-format Helvetica in DOCX styles AND EPUB CSS). **Verified live in the studio:** "Large print" clicked → stored → export **90 → 114 pages (+27%)**; the heading-font select merged (`{preset:'large', headingFont:'sans'}`); Reset → cleared, **back to exactly 90**; zero console errors. Build notes: a merge that leaves every field unset degrades to a CLEAR (never the `{}` the backend rejects); two pre-existing test-selector collisions with the new "Standard" button resolved consciously (selector-scoped, both words right in their own sections). Queue item 4 closed.
