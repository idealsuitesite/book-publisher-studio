...
**Status:** 🔍 **FINDINGS — awaiting CTO. No code changed.** Ordered by the CTO (2026-07-21, high priority — first-contact trust) after landing on the "Could not open this project" error as the FIRST screen, reaching the real library only after clicking "Back to the studio". Same discipline as the fidelity investigations: reproduce live, locate, classify — and, per the standing rule, the CTO's mechanical hypothesis was **verified against the code, not assumed correct**.
