...
**Phase B — ✅ IMPLEMENTED (2026-07-19, CTO-ordered "ensuite la phase B").** Scope as designed below, with two adjustments made during implementation and disclosed: splitting applies to plain paragraphs only (quotes/scriptures render with a first-line indent whose continuation semantics differ; drop-cap paragraphs' opening lines are typographically special — all stay atomic), and `Page` gained `splitAfterLines`/`startsWithContinuation` rather than a span type, because every existing consumer (TOC, running heads, both renderers' page starts) reads `blocks: string[]` and a compatible extension kept the change reviewable. Lists and tables splitting at item/row boundaries remain future work, recorded here. The end-to-end proof: a really-split 700-word paragraph renders with the model and the shipped PDF agreeing on the exact page count.

*Original design (kept for the record):* line-level block splitting. `Page.blocks` entries become spans (`{ blockId, fromLine?, toLine? }`), `LayoutEngine` fills the remainder of a page with the head of the next paragraph when at least 2 lines fit and at least 2 lines remain for the following page — which **is** widow/orphan control (min-2-lines at both ends), plus `staysWithNext` generalized so a heading never sits last on a page. The renderer renders spans (PDFKit `heightOfString` per line range). Lists, quotes, tables split at item/row boundaries; images never split. This is the CTO's "veuves, orphelines, paragraphes longs, titres, listes, citations, images" list, mapped one-to-one — and it needs its own commit series with real-book visual verification, which is why it is not squeezed into tonight.
