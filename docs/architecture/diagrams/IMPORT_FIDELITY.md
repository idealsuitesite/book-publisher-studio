...
**✅ Commit 3 EXECUTED (2026-07-20).** `ApiErrorDTO`/`ApiErrorCode` in shared-types (`code` additive, `error` string kept); every `errorHandler` branch and all four `ProjectsController` 404s now name their error (`PROJECT_NOT_FOUND`, `IMPORT_PARSE_FAILED` — moved to 422, the file is the problem, not the transport — `UNKNOWN_FORMAT`, `UNKNOWN_OPTION`, `UPLOAD_REJECTED`, `INTERNAL`); export/publish exceptions answer `RENDER_FAILED` with the real cause in the server log, never in the author's face. Frontend: `ApiError` joins timeout/network as the third failure family; `PROJECT_NOT_FOUND` shows "This project is no longer in your library," **clears the stale resume-where-left entry**, and the "projects live in memory" copy — false since ADR-0048 — is dead. The provisional KDP gate (`StructurePresenceRule`, CTO amendment) blocks publish at zero chapters above the threshold while export stays diagnostic. **One real bug found live, fixed, and locked:** the rule first read `book.wordCount`, but the publish path rebuilds the book *without* metrics enrichment — the enriched field silently read 0 and the rule never fired; synthetic tests that hand-set the field passed. Now `countBookWords()` counts from the AST itself, shared with `BookValidator`, and the fixtures carry their words in real paragraph text. Proven live: stale id → `{code: PROJECT_NOT_FOUND}` + typed UI screen; unstructured project → KDP `FAIL` with the named ERROR while export returns 200. Backend 555/555, frontend 138/138, builds + lint clean.

**✅ Commit 4 EXECUTED (2026-07-20).** The only two internal references that ever rendered were the Inspector's layout rows — "not yet applied (ADR-0043)" and "planned (LAYOUT_FIDELITY)" — now "Coming soon" (the warn styling stays: honesty in the author's language, ticket ids in the code where the next engineer looks). Baseline deliberately recaptured: exactly one screen changed (`04-layout-kdp--desktop`), byte-identical twice after.

**✅ Commit 5 EXECUTED (2026-07-20) — ALL FIVE COMMITS DONE, `verify-real-import` GREEN ON THE CORPUS.** `backend/verification/corpus/` + `npm run verify-real-import` (real server, per-file assertions): `faith-alone-styled.docx` — 17 chapters exact, 39,913 words exact, **`EMPTY_HEADING_DROPPED` as a permanent assertion** (the CTO's condition — the 18 h1 → 17 discovery can never regress silently), no false `UNSTRUCTURED_MANUSCRIPT`; `pm-notes-unstyled-fr.docx` — 0 chapters, 1,424 words, silent below the threshold (the Q3 semantic, asserted); `generated-unstyled-3060w.docx` — `UNSTRUCTURED_MANUSCRIPT` must fire, structure score must be < 100, project must still be created (disclosed as generated — a real book-length unstyled manuscript replaces it the day one lands, per `REAL_FIXTURE_POLICY.md`). Every real defect this review found now has a line in this corpus that fails loudly if it returns. The CTO's freeze condition — the five commits done and the harness green — is met on the branch; the merge to `main` is the remaining gate.
