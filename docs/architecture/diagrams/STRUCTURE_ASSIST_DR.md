...
> **⏩ DELIVERY STATUS (2026-07-23): BUILT, MERGED, CLOSED.** The CTO validated delivery and ordered the merge. The chantier is **merged to `main` (`--no-ff` `2c40d71`, 4 atomic gated commits — Domain foundation + the bidirectional invariant `eb8d5ff`, read-only API `5d067c3`, review panel `77f053a`, the gesture-counter judge `5520885`); `feat/structure-assist` deleted.** **Post-merge gate ON MAIN: backend 838/838, frontend 218/218, builds + tsc + eslint clean; live harnesses 4/4 · 16/16 · 4/4** (the live gate also surfaced and re-locked a stale `<br>`-merge word-count assertion, `6ac3c89`, +42 = one word per `<br>`, attributed not softened — unrelated to this chantier's import-untouching scope). **§5's judge, both poles: 14→1 under (precision 100%, zero post-confirm corrections), silent over.** **Fixture policy SETTLED (CTO, 2026-07-23), not open:** the invariant is tested mechanically on synthetic poles (committed, CI-repeatable) AND behaviourally on the real founder books (probe) — the retained architecture, NOT a stopgap; the founder's manuscripts are NOT committed to the corpus (a versement would be his explicit decision, never a chantier side effect). Named `PRIVATE_MANUSCRIPT_FIXTURES`. The body below is the DR as validated; the ≈7→≈1 target in §5 was re-measured to a true **14→1** on the current state (the books moved, #7). **NEXT: `STRUCTURE_CLEANUP` (the §9 sibling) → `AUTHOR_EXPERIENCE` → `TYPOGRAPHY_QUALITY`.**

**Date:** 2026-07-22 · **Updated 2026-07-23** with the CTO's decisions (D1–D4 settled, the
bidirectional invariant, `STRUCTURE_CLEANUP` named as the sibling capability) · **Status: VALIDATED
& MERGED 2026-07-23 (see the delivery banner above).** *(It was re-submitted for CTO
re-validation before any code per non-negotiable #4 — a Design Review that moved is re-validated;
these decisions changed the *nature* of the chantier — then validated and built.)* The core of the founder's dream. Full DR, not a mini: a new capability (structure
