...
## Implementation note (added at build time — the design above is unchanged except the disclosed §4.1 amendment; this records what the build settled)

Built on `feature/part-level` in five commits, gate green at each (backend 674 → **709**, frontend 181 → **188**, tsc + eslint clean both sides throughout).

- **Commit 1 (`d91d842`) — the drift fix, both sides of the invariant** (the §4.1 amendment, disclosed before the commit): `ownsBarePage` in `LayoutEngine.walkContent` (page carrying the content's own id, chapter-opening blank-page protocol, titleHeightOf charged) + `PDFRenderer.renderContent`'s `startKey` fallback + the TOC content-id fallback. Locked by `partOpenerParity.test.ts` on real faith-alone: base **155/161/3 unchanged** (corpus byte-identity), +3 openers → **158/165/4** (the 3 opener drifts gone — planned; the +1 is the known ±1-line bold-run residual, attributed; the ADR-0051 ledger `real = model + 3 front pages + unplanned` balances in both runs). Unit tests pin every excluded shape (empty-with-sections, blockless-untitled, top-level section, ordinary chapter) to its pre-fix behaviour.
- **Commit 2 (`f332c0a`) — domain.** `Chapter.partOpener?: true`; `insertPartOpener`/`removePartOpener` (pure, renumber-preserving); `renumberChapters` skips openers (continuous numbering, CTO rule); **an interaction guard found by reading**: `mergeChapterIntoPrevious` refuses an opener as target (pouring blocks into a divider would silently un-divider it).
- **Commit 3 (`36c24c0`) — boundary.** `ChapterDTO.partOpener` (shared-types) + mapper; `StructureMutation` + dispatch + **`parseMutation` whitelist with route tests in the same commit** (the `setPartRole` live-gap lesson, not repeated).
- **Commit 4 (`0b08f0b`) — studio.** Divider rows (Part badge, inline rename, Remove divider; no number/count/merge/placement); per-row "+ Part" inserting above (default title, renamed inline — the promote precedent); the §6 wording collision resolved (the footer counts what things ARE — "1 part · 2 chapters" — old "parts"-as-entries wording retired with its test updated); `bookFacts` excludes dividers by tag. **§3.6 with one honest correction:** insert → a genuine cache MISS (proven); remove → a **legitimate HIT** — it restores byte-identical pre-insert content whose geometry is correctly cached (same content = same geometry; the content-hash key cannot confuse two states that ARE the same state) — asserted with the reasoning, not forced into a false "always miss".
- **Commit 5 — verified LIVE in the studio on real faith-alone** (the §4.5 sequence): "+ Part" before Chapter One → divider lands between INTRODUCTION and Chapter One, snapshot v1; inline rename → "Part I: Faith", v2; **"Chapter 2" stayed "Chapter 2" throughout** (continuous numbering live); DOCX export order **INTRODUCTION < Part I: Faith < Chapter One**; PDF export **91 pages with the divider → 90 after "Remove divider" — exactly +1 page, charged==consumed observed live**; remove → chapters flow back, v3; zero console errors; `verify-real-export` 16/16, `verify-real-import` 4/4.
- **Deferred set unchanged** (§6): per-part restart, group-drag, part-in-running-heads, opener typography, TOC indentation, the blockless-titled *section* shape.

**Status: complete on `feature/part-level`, all green, awaiting the CTO's merge decision** (GR-1). `CURRENT_STATE.md`/`TODO.md`/`VERSIONS.md` reconcile after merge.
