...
**Status: CLOSED — MERGED TO `main` (`--no-ff` merge `d6e0cd5`, 2026-07-24); `feat/incremental-render` deleted. Founder taste-stop CLOSED (surface validated → one regression found by the founder → fixed on-branch → re-validated by the founder: zoom fluid, fit-width working and fast).** Post-merge gate on `main`: backend 921/921, frontend 238/238, tsc + eslint + both builds clean; verify-real-* 4/4·16/16·4/4 on a throwaway server (zero trace); real store verified at the 9-project baseline (incl. the founder's fresh import). Candidate 3 stays consigned second-order. Carried forward as AUTHOR_EXPERIENCE inputs: the criterion-B (contextual editing / structure-editing-from-Proof) demand and `EMPTY_CHAPTER_FROM_STRUCTURE_EDIT`. The AUTHOR_EXPERIENCE Design Review opens on the CTO's word after he reviews the merged state.
