...
**PROGRESS (2026-07-25): C6 + C7 + C8 DONE; C9 PARTIAL (items 1+3 done + gated).** C9 items 2+4 and C10 remain. The M3 INPUTS below fold in.
> - **C9 item 1 (constat) ✅** — the static "Conclusion" attributed (a subsection title; `SUBSECTION_EDITING` consigned — see M3 INPUTS below).
> - **C9 item 3 ✅ (`a22a62a`)** — no-op `setPartRole` writes no version (referential no-change → no version, no save) + the placement control renamed to author language ("Where it appears in the book" / Front matter · Main body · Back matter, with an effect line). Backend 967/967, frontend 255/255.
> - **C9 REMAINING — resume here:** **item 2** the D5 real-page theme thumbnails (the founder picks the graphic language on an engine-rendered page — C8's separator/drop cap now on display; cached/pre-rendered, never blocking) — **THIS is the founder taste-stop, built to the point of showing**; **item 4** Proof-follows-the-edit (after a gesture the Proof scrolls/highlights the re-inked region, using P1's region machinery); **+ the CTO-added SUBSECTION BOUNDARY AFFORDANCE** (one sentence of UI honesty when a subsection title / a block inside a subsection is selected — see M3 INPUTS item 3; surface-level, not the consigned `SUBSECTION_EDITING`). Then **C10** (D6 title surface — read the inherited C8 ruling before adding any field — + light-default theme toggle).
...
> - **C8 (`416119a`)** — D5 the scene-break **separator becomes a theme value**, rendered consistently tri-format (`Theme.presentation.separator`; Classic rule, Modern/Novel asterisks; the ONE `resolveSeparatorStyle`). Fixes real dead/divergent code (`Divider.style` was read by no renderer; PDF/DOCX drew `* * *`, EPUB always `<hr>`). **Parity held with NO re-lock** — every style consumes exactly one `line` (what LayoutEngine charges), so pagination is unchanged; only the glyph differs. Backend 965/965, tsc + eslint clean, verify-real-export 16/16.
>   - **#7 finding + CTO ruling (recorded for C10 to inherit):** the chapter opening is ALREADY theme-controlled — the mockup's per-theme opening difference decomposes exactly into **drop cap** (`DropCapPresentation`, already a theme value) **+ title treatment** (D6's surface, C10's work). An opening-ornament field in C8 would have been a third representation of a distinction the theme already expresses through its two real mechanisms, so **no field was added.** **RULING (CTO 2026-07-25): correct — no field now.** **C10 inherits this:** if C10's title-treatment work surfaces a genuine third dimension the theme cannot express through drop-cap + title-treatment, THAT is the moment to add a field — **measured need, not symmetry.**
...
3. **A static, non-selectable "Conclusion" block** — **ATTRIBUTED (constat, 2026-07-25, C9): a SUBSECTION TITLE.** Read-only on his active copy (`6t6pypzon`) the recurring "Conclusion" markers are **subsections** (each chapter's `section[0]`), so the workspace renders the section title as a static `<h3>` (`ContentBody`, non-selectable **by design** — section titles are not "Convert to…" targets) and a block *inside* a subsection cannot be promoted (`promoteToChapter` is scope-locked to **top-level containers**, CREATE_CHAPTER round 1 — the source of his "no promotable text block in a top-level container" error, a SHARED root, not a neighbour). The raw error itself is **already C7-fixed** (author-language). **VERDICT: the editing capability is DEEPER — a documented scope-lock, not a C9 selection/UI gap → CONSIGNED as `SUBSECTION_EDITING` (a named follow-up): give subsections a contextual action (promote-to-chapter / rename / the "Conclusion"-family handling) in the document centre; needs a new op (section→chapter) and lifts CREATE_CHAPTER's round-1 nested boundary. Not built in M3.** A possible refinement noted for it: the C7 `CONTENT_NOT_FOUND` message ("isn't there any more") is slightly off for the not-promotable-from-a-subsection case — distinguishing it needs a distinct backend reason code, also part of the follow-up.
   - **CTO RULING (2026-07-25) — a minimal BOUNDARY AFFORDANCE is IN C9's remaining scope (surface-level, NOT the deep capability).** The acceptance criterion ("no gesture leaves the author wondering") does not require the capability — it requires the *communication*: the founder will hit this boundary again in his very next session (click a subsection's "Conclusion", find it inert, wonder). So C9 adds **one sentence of UI honesty**: when the selection or click lands on a **subsection title** (or a **block inside a subsection**), the workspace says in author language what is true — e.g. *"This is a section of chapter N — sections can't be converted yet"* — instead of silence or a dead click. This closes the traversal finding at the surface; `SUBSECTION_EDITING` (the deep capability) waits its turn. Build it with C9 items 2/4.
