...
3. **Render micro-optimisation — CLOSED ON EVIDENCE (2026-07-21, `backend/spikes/render-decomposition-spike.ts`; the "Savoir avant de toucher" measurement, read-only).** The ~380ms render decomposes, measured on faith-alone/kdp-6x9/classic at production settings (158 pages, warm median): **`doc.text()` ~240ms (~75%, 1172 calls)** — PDFKit's own line-breaking + glyph layout, the inherent work of drawing the book; **`doc.end()` ~36ms (~11%)** — font subsetting + zlib, inherent to emitting the file; headers/footers ~20ms; and the sum of every candidate micro-target — font state changes **4.1ms**, page ops **0.9ms**, graphics state **4.9ms** — is **~10ms**, the only slices a micro-optimisation could safely touch. Cutting the dominant cost would mean bypassing PDFKit's text pipeline with pre-measured line placement — core surgery on the parity-locked component (the exact §3-forbidden class) for a bounded ~240ms. **The ceiling of safe wins (~10ms, <3%) is out of all proportion to the risk surface; the option closes the same way the font cache did — on measurement, not for want of an idea.** Performance (S13) is therewith honestly complete for now: Option 1 closed on evidence, Option 2 delivered, Option 3 closed on evidence; reopen only on a real user latency signal (then the §5.5 streaming question folds into that review, not before).
...
- **Option 2 (pagination reuse on colour-only refresh) — BUILT & MERGED (`MINI_DR_PAGINATION_REUSE.md`, merge `bfc756f`).** The reused artifact costs **~50 ms**, not ~215 ms. R2-safe by the proven accent invariant; worth it because the accent picker is a *drag-to-tune* control (many refreshes in a row), where 50 ms × N adds up and the reuse is provably free. Delivered: a `PaginationCache` port + in-memory LRU (cap 16), `renderBook` reuse on a `md5(book):theme:layout` key (accent excluded), the four invalidation tests + the `pages[]` accent-invariance guard, verified live in production (accent change → different PDF bytes, re-inked not stale).
