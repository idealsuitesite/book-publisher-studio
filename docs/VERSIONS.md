...
| v0.6.0-alpha | Validation Engine | ⏳ Planned (implementation complete, tag not yet cut) | — | `ValidationEngine` orchestrating a `RuleRegistry` of 8 independent rules (structural, metadata, heading hierarchy, typography, images, hyperlinks, KDP/EPUB pre-render readiness), `QualityScore` composite scoring, wired into `ImportManuscriptUseCase` — Sprint 5, all 11 commits done, 282/282 tests, `npm run verify-real-export` 16/16. See `docs/releases/v0.6.0-alpha/SPRINT_5_FINAL_REPORT.md`. **This row's milestone was corrected from an earlier, superseded assumption:** it previously read "Premium UI/UX" (guessed before Sprint 5's actual scope was decided by its own Design Review, `docs/architecture/diagrams/PLATFORM_ARCHITECTURE_ROADMAP.md`) — that row's content moved to v0.7.0-alpha below, and every subsequent row shifted down one version accordingly, since none of them had been released under their old number. Tag/PR merge pending. |
