...
| v0.4.0-alpha | PDF Export | 🔶 Merged to `main`, tag not yet cut | — | `PDFRenderer` (PDFKit, ADR-0014), `POST /api/manuscripts/export` gains a `format=pdf` field (PR #3, merge commit `820f1ef`). 125 tests passing (up from 118). See `docs/releases/v0.4.0-alpha/ReleaseNotes.md`. Originally scoped as "Professional Export" covering both PDF and EPUB together; split in two after PDF landed first per the CTO-directed Sprint 3A/3B re-sequencing — EPUB is now its own row below. |
| v0.4.1-alpha | EPUB Export | ⏳ Planned | — | EPUB library spike + ADR-0015 resolution (`epub-gen` vs. hand-rolled OOXML via `jszip`) first, then `EPUBRenderer` — Sprint 3B scope. |
