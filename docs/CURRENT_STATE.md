...
**Last Updated:** July 18, 2026 — **Sprint 8 ("Publishing Engine") implementation complete on `feature/sprint-8-publishing-engine`** (Commits 0–8). **Not merged, not tagged, no PR opened yet.** Sprint 7 remains the last released version (`v0.8.0-alpha`).
...
**Branch:** `feature/sprint-8-publishing-engine` (active, pushed to origin), branched from `main` at `6d53914`. **No PR opened, not merged, not tagged** — `main` remains at `v0.8.0-alpha`. `feature/sprint-7-first-demonstrable-product` was deleted (local + remote) after PR #12 merged.
...
## Sprint 8: Publishing Engine 🟡 IMPLEMENTATION COMPLETE — not merged, not tagged

**Branch:** `feature/sprint-8-publishing-engine`. **Design Review ✅ APPROVED** (`docs/architecture/diagrams/PUBLISHING_ENGINE.md`) — 8 decisions locked across two rounds plus four mid-sprint CTO reviews, each recorded with the CTO's own reasoning, not just the ruling.
...
**Not done, deliberately:** no PR, no merge, no tag, no `VERSIONS.md` flip — those are `docs/RELEASE_CHECKLIST.md` steps requiring their own explicit authorization. No frontend work: Sprint 8 is backend-only, so `POST /api/manuscripts/publish` has no UI yet.
...
**Sprint 8 (Publishing Engine) is implementation-complete but NOT released.** All 9 commits (0–8) are done and pushed to `feature/sprint-8-publishing-engine`. Still outstanding, each requiring its own explicit authorization per `docs/RELEASE_CHECKLIST.md`: open a PR, merge to `main`, tag (`v0.9.0-alpha`), write `ReleaseNotes.md` + `SPRINT_8_FINAL_REPORT.md`, flip the `docs/VERSIONS.md` row, delete the branch. **`docs/VERSIONS.md` has deliberately not been touched** — its `v0.9.0-alpha` row still reads "Plugin System / ⏳ Planned", which is now wrong and must be reconciled during release closure, not before a tag exists.

**One decision is waiting for the CTO before any future Publishing Engine work:** ADR-0038 (**OPEN**) — how to expose `LayoutEngine`'s real pagination metrics to the Publishing Engine without breaking the existing separation of responsibilities. Deferred deliberately; the ADR frames the question and lists candidate shapes without endorsing one.
...
**Branch:** `feature/sprint-8-publishing-engine` (active, pushed to origin), branched from `main` at `6d53914`. **No PR opened, not merged, not tagged.**
**`feature/sprint-8-publishing-engine` history (most recent first):** *(this docs/ADR reconciliation commit)* → `6a7dc89` (test: real-fixture verification pass, commit 7) → `9133931` (feat: `POST /api/manuscripts/publish`, commit 6) → `68be289` (feat: `PublishingUseCase`, commit 5) → `99f0df7` (feat: `KDPTarget`, commit 4) → `5abd067` (feat: `SubmissionValidator` + rule family + provider, commit 3) → `86e0116` (feat: `Packaging` + `PublishingBundle`, commit 2) → `ff3fad5` (feat: enrich `PublishingReport` + Decision 8/ADR-0037) → `2c5e434` (feat: `PublishingTarget` port + models, commit 1) → `baa48ac` (docs(spike): KDP requirements spike, commit 0) → `6d53914` (`main`: Design Review approved + internal responsibilities diagram)
**`main`:** unchanged at `6d53914`, still tagged `v0.8.0-alpha` — Sprint 8 has not been merged into it.
