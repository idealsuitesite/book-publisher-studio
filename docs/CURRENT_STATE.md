...
**Last Updated:** July 18, 2026 (Sprint 6 released as `v0.7.0-alpha`; post-Sprint-6 governance restructuring complete; Sprint 7 "First Demonstrable Product" Kickoff written, CTO go-ahead granted, branch created, Commits 1-2 done)
**Sprint:** Sprint 6 ("Professional Layout Engine") **✅ COMPLETE AND RELEASED**, tagged `v0.7.0-alpha` (merge commit `eb05beb`, PR #11, 328/328 tests, two real bugs found and fixed during real-file verification — ADR-0031/0032; see `docs/releases/v0.7.0-alpha/ReleaseNotes.md` / `SPRINT_6_FINAL_REPORT.md` for full detail). **Post-Sprint-6 governance restructuring (2026-07-18, CTO-directed):** `docs/CLAUDE.md` cut from ~135 to ~55 lines, now a thin entry point pointing to new specialized docs — `DEVELOPMENT_WORKFLOW.md`, `DESIGN_REVIEW_PROCESS.md`, `RELEASE_CHECKLIST.md`, `ADR_INDEX.md`, `DEVELOPER_HANDBOOK.md`, `QUALITY_GATE.md`, `TESTING_STRATEGY.md`, `REAL_FIXTURE_POLICY.md`. ADR-0032 extended with the **Engineering Governance Principle** (no feature is done until validated simultaneously at Code/Product/Documentation levels). **Sprint 7 ("First Demonstrable Product", renamed from "Premium UI/UX") is now IN PROGRESS.** Design Review ✅ APPROVED round 2 (`docs/architecture/diagrams/SPRINT_7_FIRST_DEMONSTRABLE_PRODUCT.md`) — all 5 open decisions locked by explicit CTO direction (full-re-export preview, stateless backend, minimal-for-demo scope, `packages/shared-types` npm workspace, an extensible `GET /api/manuscripts/options` endpoint). New `docs/product/` documentation layer written (`PERSONAS.md`, `USER_JOURNEYS.md`, `FEATURE_MATRIX.md`, `WIREFRAMES.md`, `PRODUCT_DEMO.md` with the official Demo Script, `PRODUCT_ACCEPTANCE.md`) plus `docs/demo/screenshots/README.md`. **`docs/architecture/diagrams/SPRINT_7_KICKOFF.md` written** (CTO-directed short charter between Design Review and Commit 1, matching the Sprint 6 Kickoff precedent) — also introduced a durable `docs/DEVELOPMENT_WORKFLOW.md` rule: every `frontend/`-touching commit from Sprint 7 onward must ship something visibly working. **CTO go-ahead granted 2026-07-18**; `feature/sprint-7-first-demonstrable-product` branched from `main`. **Commits 1-2 done:** repository converted to an npm workspace, `packages/shared-types` scaffolded (ADR-0033); `GET /api/manuscripts/options` shipped (Decision 5), including a CTO-resolved design tension between "no Domain changes" and "no duplicated registries" — see Sprint 7 section below for full detail.
**Branch:** `feature/sprint-7-first-demonstrable-product`, at `6ded695`, 4 commits ahead of `main` (`main` at `31b3139`, tag `v0.7.0-alpha`).
...
**Remaining commits (per `SPRINT_7_FIRST_DEMONSTRABLE_PRODUCT.md` §6 / `SPRINT_7_KICKOFF.md`):** 3 (DTOs re-exported from `packages/shared-types`), 4 (frontend API client), 5-9 (upload/structure/validation/format-selector/export-preview UI, each required to ship something visibly working per the new frontend-commit rule), 10 (real-file verification pass), 11 (screenshots), 12 (docs/ADR reconciliation + Sprint 7 Final Report). None started yet.
...
**Sprint 7 ("First Demonstrable Product") is IN PROGRESS on `feature/sprint-7-first-demonstrable-product`.** Design Review ✅ APPROVED (round 2): `docs/architecture/diagrams/SPRINT_7_FIRST_DEMONSTRABLE_PRODUCT.md` (now with a formal Decision Index — cite "Decision 1" through "Decision 5" instead of quoting paragraphs, CTO direction). Kickoff charter written per CTO direction: `docs/architecture/diagrams/SPRINT_7_KICKOFF.md` (also introduced the durable "every frontend commit ships something visible" rule, now in `docs/DEVELOPMENT_WORKFLOW.md`). A monorepo repository-structure diagram was also added to `docs/DEVELOPER_HANDBOOK.md` (CTO request). CTO go-ahead granted 2026-07-18. **Commit 1 done** (npm workspace conversion + `packages/shared-types` scaffold, ADR-0033). **Commit 2 done** (`GET /api/manuscripts/options`, Decision 5 — see the Sprint 7 section above for full detail, including the CTO-resolved Domain-touch tension). **Next action: Commit 3** — `feat(backend): DTOs re-exported from packages/shared-types` (`BookDTO`, `ImportReportDTO`, `ValidationIssueDTO`, `QualityScoreDTO`), no behavior change, verified via the existing test suite staying green.
...
**Branch:** `feature/sprint-7-first-demonstrable-product`, at `6ded695` (Commit 2: `GET /api/manuscripts/options`, Decision 5), branched from `main` at `31b3139`. `main` itself is at `31b3139`, tag `v0.7.0-alpha` (on an earlier commit, `eb05beb` — `main` has moved on with docs-only commits since).
...
**`feature/sprint-7-first-demonstrable-product` history (most recent first):** `6ded695` (Commit 2 — `GET /api/manuscripts/options`) → `f286a46` (Decision Index + repository structure diagram) → `cb04678` (Git Status hash fix) → `583522c` (Commit 1 — npm workspace conversion, `packages/shared-types` scaffold, ADR-0033) → branched from `main` at `31b3139`.
