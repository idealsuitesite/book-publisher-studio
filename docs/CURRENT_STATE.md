...
**No sprint is currently assigned.** `docs/architecture/diagrams/PLATFORM_ARCHITECTURE_ROADMAP.md` §2 still maps Editorial AI Engine and Plugin System with real dependencies fixed, but with no ordering decided between them — that is each engine's own future Design Review to resolve, one at a time, same discipline as every sprint so far.
