...
**Last Updated:** July 18, 2026 (Sprint 6 released as `v0.7.0-alpha`; post-Sprint-6 governance restructuring complete; Sprint 7 "First Demonstrable Product" Kickoff written, CTO go-ahead granted, branch created, Commits 1-4 done — Commit 4 shipped the first real UI screen)
**Sprint:** Sprint 6 ("Professional Layout Engine") **✅ COMPLETE AND RELEASED**, tagged `v0.7.0-alpha` (merge commit `eb05beb`, PR #11, 328/328 tests, two real bugs found and fixed during real-file verification — ADR-0031/0032; see `docs/releases/v0.7.0-alpha/ReleaseNotes.md` / `SPRINT_6_FINAL_REPORT.md` for full detail). **Post-Sprint-6 governance restructuring (2026-07-18, CTO-directed):** `docs/CLAUDE.md` cut from ~135 to ~55 lines, now a thin entry point pointing to new specialized docs — `DEVELOPMENT_WORKFLOW.md`, `DESIGN_REVIEW_PROCESS.md`, `RELEASE_CHECKLIST.md`, `ADR_INDEX.md`, `DEVELOPER_HANDBOOK.md`, `QUALITY_GATE.md`, `TESTING_STRATEGY.md`, `REAL_FIXTURE_POLICY.md`. ADR-0032 extended with the **Engineering Governance Principle** (no feature is done until validated simultaneously at Code/Product/Documentation levels). **Sprint 7 ("First Demonstrable Product", renamed from "Premium UI/UX") is now IN PROGRESS.** Design Review ✅ APPROVED round 2 (`docs/architecture/diagrams/SPRINT_7_FIRST_DEMONSTRABLE_PRODUCT.md`) — all 5 open decisions locked by explicit CTO direction (full-re-export preview, stateless backend, minimal-for-demo scope, `packages/shared-types` npm workspace, an extensible `GET /api/manuscripts/options` endpoint). New `docs/product/` documentation layer written (`PERSONAS.md`, `USER_JOURNEYS.md`, `FEATURE_MATRIX.md`, `WIREFRAMES.md`, `PRODUCT_DEMO.md` with the official Demo Script, `PRODUCT_ACCEPTANCE.md`) plus `docs/demo/screenshots/README.md`. **`docs/architecture/diagrams/SPRINT_7_KICKOFF.md` written** (CTO-directed short charter between Design Review and Commit 1, matching the Sprint 6 Kickoff precedent) — also introduced a durable `docs/DEVELOPMENT_WORKFLOW.md` rule: every `frontend/`-touching commit from Sprint 7 onward must ship something visibly working. **CTO go-ahead granted 2026-07-18**; `feature/sprint-7-first-demonstrable-product` branched from `main`. **Commits 1-4 done:** repository converted to an npm workspace, `packages/shared-types` scaffolded (ADR-0033); `GET /api/manuscripts/options` shipped (Decision 5); the 9 pre-existing backend DTOs migrated to `packages/shared-types` with a new durable "transport contracts only" rule (ADR-0033 addendum); Book Publisher Studio's first real UI screen shipped — title + static drop zone, Commit 4 redefined by explicit CTO direction — see Sprint 7 section below for full detail.
**Branch:** `feature/sprint-7-first-demonstrable-product`, at `2046711` (Commit 3: DTOs re-exported from `packages/shared-types`, plus the Visible Increment Rule scope extension), branched from `main` at `31b3139`, tag `v0.7.0-alpha`.
...
**Remaining commits (per `SPRINT_7_FIRST_DEMONSTRABLE_PRODUCT.md` §6 / `SPRINT_7_KICKOFF.md`):** 5 (upload flow — wires the dropzone to the real import call), 6-9 (structure/validation/format-selector/export-preview UI, each required to produce a real Visible Increment entry), 10 (real-file verification pass), 11 (screenshots), 12 (docs/ADR reconciliation + Sprint 7 Final Report). None started yet.
...
**Sprint 7 ("First Demonstrable Product") is IN PROGRESS on `feature/sprint-7-first-demonstrable-product`.** Design Review ✅ APPROVED (round 2): `docs/architecture/diagrams/SPRINT_7_FIRST_DEMONSTRABLE_PRODUCT.md` (now with a formal Decision Index — cite "Decision 1" through "Decision 5" instead of quoting paragraphs, CTO direction). Kickoff charter written per CTO direction: `docs/architecture/diagrams/SPRINT_7_KICKOFF.md`. A monorepo repository-structure diagram was also added to `docs/DEVELOPER_HANDBOOK.md` (CTO request). CTO go-ahead granted 2026-07-18. **Commit 1 done** (npm workspace conversion + `packages/shared-types` scaffold, ADR-0033). **Commit 2 done** (`GET /api/manuscripts/options`, Decision 5). **Visible Increment Rule formalized, then scope-extended (CTO direction):** every implementation commit from Commit 3 onward is logged in `docs/demo/VISIBLE_INCREMENTS.md` — a real screenshot + description + real-data confirmation for `frontend/`-touching commits, a small SVG diagram + the same description/confirmation for backend/tooling commits. **Commit 3 done** (DTOs re-exported from `packages/shared-types`, plus the "transport contracts only" package-scope rule, ADR-0033 addendum). **Commit 4 done — the first real UI screen** (CTO redefined its scope from headless API-client-only to Book Publisher Studio's actual first screen: title + static drop zone + `lib/api-client.ts` bundled in; see the Sprint 7 section above for full detail, including the disclosed screenshot-file-persistence gap and the `docs/demo/screenshots/` naming collision found and reconciled). **Next action: Commit 5** — `feat(frontend): upload flow` (wire the dropzone to a real `POST /api/manuscripts/import` call, loading/error states).
...
**Branch:** `feature/sprint-7-first-demonstrable-product`, at `2046711` (Commit 3: DTOs re-exported from `packages/shared-types`, ADR-0033 addendum), branched from `main` at `31b3139`. `main` itself is at `31b3139`, tag `v0.7.0-alpha` (on an earlier commit, `eb05beb` — `main` has moved on with docs-only commits since).
