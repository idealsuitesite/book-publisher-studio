...
**Last Updated:** July 18, 2026 (Sprint 6 released as `v0.7.0-alpha`; post-Sprint-6 governance restructuring complete; Sprint 7 "First Demonstrable Product" Kickoff written, CTO go-ahead granted, branch created, Commits 1-10 done, Commit 9 split into 9a/preview and 9b/export at CTO direction, Commit 10 redefined by the CTO as a "Sprint Review Technique" — a 7-part real-file/real-rebuild verification pass, no code changes, confirming Sprint 7 is genuinely reproducible from a cold start)
...
**Branch:** `feature/sprint-7-first-demonstrable-product`, at `5ae81f4` (Commit 10: Sprint Review Technique verification pass), branched from `main` at `31b3139`, tag `v0.7.0-alpha`.
...
**Remaining commits (per `SPRINT_7_FIRST_DEMONSTRABLE_PRODUCT.md` §6 / `SPRINT_7_KICKOFF.md`):** 11 (Polish UI — states/messages/ergonomics/light animation, now including the CTO's this-session UI feedback below), 12 (curated screenshots, docs/ADR reconciliation, `docs/demo/VISIBLE_INCREMENTS.md` compiled into `SPRINT_7_TIMELINE.md`, Sprint 7 Final Report).
...
**Branch:** `feature/sprint-7-first-demonstrable-product`, at `5ae81f4` (Commit 10: Sprint Review Technique verification pass, docs-only, self-referential — this line was written after the commit that produced it), branched from `main` at `31b3139`. `main` itself is at `31b3139`, tag `v0.7.0-alpha` (on an earlier commit, `eb05beb` — `main` has moved on with docs-only commits since).
...
**`feature/sprint-7-first-demonstrable-product` history (most recent first):** `5ae81f4` (Commit 10 — Sprint Review Technique verification pass, docs-only) → `8758497` (docs: log Commit 9b, sync CURRENT_STATE.md/TODO.md) → `eac9fec` (Commit 9b — export/download) → `2d63c0b` (docs: log Commit 9a, sync CURRENT_STATE.md/TODO.md) → `3ab8926` (Commit 9a — PDF preview) → `89eb472` (docs: log Commit 8, sync CURRENT_STATE.md/TODO.md) → `1e5b392` (Commit 8 — format/layout selector) → `531ad20` (docs: log Commit 7, sync CURRENT_STATE.md/TODO.md) → `012428e` (Commit 7 — validation summary) → `78cac12` (docs: log Commit 6, sync CURRENT_STATE.md/TODO.md) → `b0865b7` (Commit 6 — book structure view) → `a94acac` (docs: record 3 CTO decisions + log Commit 5) → `27e4780` (Commit 5 — upload flow) → `f09347a` (docs: log Commit 4, reconcile screenshots dir) → `e026536` (Commit 4 — home screen + API client) → `eb5e2e3` (docs: sync with Commit 3) → `2046711` (Visible Increment Rule scope extension + Commit 3 diagram) → `5e0e561` (Commit 3 — DTOs re-exported from `packages/shared-types`, ADR-0033 addendum, `DEVELOPER_HANDBOOK.md` rule 5) → `29d3be4`/`e8cf68f` (Visible Increment Rule formalized) → `a1520fb` (hash fix) → `6ded695` (Commit 2 — `GET /api/manuscripts/options`) → `f286a46` (Decision Index + repository structure diagram) → `cb04678` (hash fix) → `583522c` (Commit 1 — npm workspace conversion, `packages/shared-types` scaffold, ADR-0033) → branched from `main` at `31b3139`.
