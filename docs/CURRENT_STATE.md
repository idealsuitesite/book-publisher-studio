...
**Branch:** `feature/sprint-7-first-demonstrable-product`, at `<COMMIT12_HASH>` (Commit 12: Sprint 7 closure), branched from `main` at `31b3139`, tag `v0.7.0-alpha`.
...
**Branch:** `feature/sprint-7-first-demonstrable-product`, at `06c9e1b` (Commit 11: Polish UI; this docs-sync commit follows immediately on top, hash not yet known at the time this line was written), branched from `main` at `31b3139`. `main` itself is at `31b3139`, tag `v0.7.0-alpha` (on an earlier commit, `eb05beb` — `main` has moved on with docs-only commits since).
...
**`feature/sprint-7-first-demonstrable-product` history (most recent first):** `06c9e1b` (Commit 11 — polish UI) → `c6baedd` (docs: fix Commit 10's own hash reference) → `5ae81f4` (Commit 10 — Sprint Review Technique verification pass, docs-only) → `8758497` (docs: log Commit 9b, sync CURRENT_STATE.md/TODO.md) → `eac9fec` (Commit 9b — export/download) → `2d63c0b` (docs: log Commit 9a, sync CURRENT_STATE.md/TODO.md) → `3ab8926` (Commit 9a — PDF preview) → `89eb472` (docs: log Commit 8, sync CURRENT_STATE.md/TODO.md) → `1e5b392` (Commit 8 — format/layout selector) → `531ad20` (docs: log Commit 7, sync CURRENT_STATE.md/TODO.md) → `012428e` (Commit 7 — validation summary) → `78cac12` (docs: log Commit 6, sync CURRENT_STATE.md/TODO.md) → `b0865b7` (Commit 6 — book structure view) → `a94acac` (docs: record 3 CTO decisions + log Commit 5) → `27e4780` (Commit 5 — upload flow) → `f09347a` (docs: log Commit 4, reconcile screenshots dir) → `e026536` (Commit 4 — home screen + API client) → `eb5e2e3` (docs: sync with Commit 3) → `2046711` (Visible Increment Rule scope extension + Commit 3 diagram) → `5e0e561` (Commit 3 — DTOs re-exported from `packages/shared-types`, ADR-0033 addendum, `DEVELOPER_HANDBOOK.md` rule 5) → `29d3be4`/`e8cf68f` (Visible Increment Rule formalized) → `a1520fb` (hash fix) → `6ded695` (Commit 2 — `GET /api/manuscripts/options`) → `f286a46` (Decision Index + repository structure diagram) → `cb04678` (hash fix) → `583522c` (Commit 1 — npm workspace conversion, `packages/shared-types` scaffold, ADR-0033) → branched from `main` at `31b3139`.
