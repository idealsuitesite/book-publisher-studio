...
**Branch:** `main`, at `<TAG_COMMIT_HASH>` (post-merge docs commit), tag `v0.8.0-alpha`. No active feature branch — `feature/sprint-7-first-demonstrable-product` deleted (local + remote) after PR #12 merged.
...
**Next action: Sprint 8 Design Review**, per the user's own direction after Sprint 7's release. No Sprint 8 scope has been decided yet — `docs/architecture/diagrams/PLATFORM_ARCHITECTURE_ROADMAP.md` §2 maps the remaining Level-1 engines (Editorial AI, Plugin System, Publishing Engine) with real dependencies fixed but no ordering decided among them; `docs/TODO.md`'s backlog also carries the licensing/monetization-architecture proposal (ADR-0034 Decision 3) and the PDF-viewer-needs-a-new-dependency item (ADR-0034 Decision 2) as real candidates raised during Sprint 7 but not yet scoped as a sprint.
...
**Branch:** `main`, at `<TAG_COMMIT_HASH>` (this docs-sync commit, following immediately on top of the merge), tag `v0.8.0-alpha`. No active feature branch.
**`main` history (most recent first):** `<TAG_COMMIT_HASH>` (docs: Sprint 7 release closure — ReleaseNotes, VERSIONS.md flip, final CURRENT_STATE/TODO reconciliation) → `f17fd65` (**merge PR #12** — Sprint 7, First Demonstrable Product, tagged `v0.8.0-alpha`) → `31b3139` (Sprint 7 Kickoff charter + frontend commit-visibility rule) → `7b8d98f` (Sprint 7 Design Review round 2 approval + `docs/product/` layer) → `acc4f9b` (Sprint 7 Design Review round 1 draft) → `ff1fcae` (CLAUDE.md restructuring + ADR-0032 Engineering Governance Principle) → `1d06643` (post-Sprint-6 Quality Gate/Testing Strategy/Real Fixture Policy docs) → `238bf37` (post-merge docs/release commit, Sprint 6) → `eb05beb` (**merge PR #11** — Sprint 6, Professional Layout Engine, tagged `v0.7.0-alpha`) → further back per prior entries.
