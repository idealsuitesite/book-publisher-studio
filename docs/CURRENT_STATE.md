...
**Branch:** `main`, at `7b8d98f`, tag `v0.7.0-alpha` (on an earlier commit, `eb05beb` — `main` has moved on with docs-only commits since). No open feature branches.
**`main` history (most recent first):** `7b8d98f` (Sprint 7 Design Review round 2 approval + `docs/product/` layer) → `acc4f9b` (Sprint 7 Design Review round 1 draft) → `ff1fcae` (CLAUDE.md restructuring + ADR-0032 Engineering Governance Principle) → `1d06643` (post-Sprint-6 Quality Gate/Testing Strategy/Real Fixture Policy docs) → `238bf37` (post-merge docs/release commit, Sprint 6) → `eb05beb` (**merge PR #11** — Sprint 6, Professional Layout Engine, tagged `v0.7.0-alpha`) → further back per prior entries.
