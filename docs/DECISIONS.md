...
**Annex (2026-07-21, founder-directed — the deferral is hardened into a suspension; the original decision above is left exactly as written, same discipline as ADR-0051's ghost-page annex):** the founder hardened this ADR's unblock condition. The Editorial AI Engine is no longer merely *deferred to Sprint 18+* on strategic-sequencing grounds — it is **SUSPENDED until the first commercial revenue-generating phase**, gated on a business fact (real revenue), **not a date and not a sprint estimate.** This is a **reinforcement of ADR-0039, not a contradiction**: the strategic ordering above still holds; a hard cost gate is layered on top, and either one alone keeps the engine from moving. The constraint's canonical definition is the Governance Record `NO_PAID_AI_BEFORE_REVENUE` (below in this file); `docs/TODO.md` → `NO_PAID_AI_BEFORE_REVENUE` is the single source of truth for the *status*, and `docs/VERSIONS.md`'s `v0.19.0-beta` row reflects it. **One narrow exception the founder authorized:** an empty, agnostic `AIProvider` port as an architectural anchor — interface only, no implementation, no `FakeAIProvider`, no wiring — reviewed in `docs/architecture/diagrams/MINI_DR_AIPROVIDER_PORT.md`. Everything else (adapters, secret management, pipeline wiring — the "items B" of `EDITORIAL_AI_STRUCTURE_SLICE.md` §3) stays suspended with the rest.
...
**Annex (2026-07-21, CTO-directed — latent empty-title drift closed, its own traceability line):** Building the subtitle-spacing chantier (`MINI_DR_SUBTITLE_SPACING.md`) revealed a **pre-existing** charged≠consumed drift, found only because the new title-spacing pair's behaviour was *verified* rather than assumed neutral: `PDFRenderer.renderTitle` spent title spacing (the old size-scaled `moveDown`) for an **empty title** — an untitled preamble Section — while `LayoutEngine.titleHeightOf` already charged it at zero (`if (!content.title) return 0`). The renderer consumed what the model never planned, the exact ADR-0051 violation; on `faith-alone` this uncharged spacing was itself producing one of the two disclosed reconciliations. **Closed** by `if (!content.title) return;` in `renderTitle`, aligning the renderer with the model. Measured, the number never rose: Classic kdp-5x8 reconciliations **2 → 1** (page count unchanged at 238, model 234 → 235 from the title-spacing charge), Modern letter **2 → 1**, Modern kdp-6x9 stays 2 (there the untitled section did not straddle a break). This is a **latent-drift closure**, not a re-lock side effect — it existed before this chantier. **Not counted in the eleven-bug real-fixture lineage** (found by verifying behaviour during the build, the same category as `DROPCAP_TEXT_OVERLAP` and `STRUCTURE_EDITS_NOT_EXPORTED`, not by a real fixture revealing what synthetic coverage missed — the lineage-count call remains the CTO's if a different ruling is wanted). Exact numbers re-locked in `PDFRenderer.parity.test.ts` and `modernTheme.test.ts`; full record in `MINI_DR_SUBTITLE_SPACING.md`.

**Annex (2026-07-22, TYPOGRAPHY_QUALITY_SCOPE §1 → MINI_DR_BLOCKLESS_TITLES — the blockless-title drift closed, its own traceability line):** The typography-quality cadrage's real-page census found a second member of the same charged≠consumed family, one level down from PART_LEVEL's blockless-chapter fix: a **titled content whose own `content[]` is empty** was invisible to title charging (`titleHeightOf` is only spent through the first block, which the shape lacks) while every renderer drew its title — model +0 / renderer +~38pt. On the real corpus the shape exists once (`"3. The Law Was a Tutor, Pointing to Christ"`, a blockless L2 section, 1 of faith-alone's 79); on Modern the drift crossed a page boundary **while rendering that title**, stranding an 18pt heading at the bottom of real p.40 — the §10.3 heading gate's "enforced by construction" claim measured false on a shipped page. **Closed** at every level (nested section, top-level section, and the chapter-with-sections-no-blocks variant, which also never planned its opening break nor claimed the running-head attribution): the title is charged in-flow with the D3 orphan guard, the page carries the content's own id (planned breaks expressible, TOC page number resolves). Measured post-fix: Modern kdp 158/2 → 157/1 (the survivor is paragraph-123's ±1-line residual), Modern letter 88/1 → 87/0, Novel 90/1 → 89/0 and 161/1 → 160/0, partOpener base 161/3 → 160/2 — every remaining reconciliation attributed, none mid-title. And the failure mode is now **nameable**: `RenderMetrics.unplannedTitleBreaks` (the D5 counter, §10.3 pinned 0 across themes by test). Like its 2026-07-21 sibling above, a latent-drift closure with its own line — found by measuring, not by luck.
...
## ADR-0052: Project Export/Publish Render the Stored Book, Not the Source Bytes — a Real Defect Found in Merged Phase-2 Code

**Status:** APPROVED — CTO-directed (2026-07-21, Option 2: this defect earns its own isolated commit *before* the front-matter Q3 commit builds on it). Fixed on `feature/structure-editing-phase2`.

**The defect, named as what it is.** Manual structure editing (phase 2 core, already merged into the branch at `8b9f65a`) stores every edit on `project.book` via `EditBookUseCase` → `replaceBook` → `save`. But `ExportProjectUseCase` and `PublishProjectUseCase` **re-parsed the retained source upload bytes on every call** (`ExportManuscriptUseCase.execute({ buffer: source.data, … })`) and rebuilt the AST from scratch — never reading `project.book`. So a reordered or renamed chapter appeared in the Structure station (which reads `project.book` through `GetProjectUseCase`) but was **silently discarded in the export and in what KDP validated.** The manuscript the author saw and the manuscript the product shipped were two different books. STRUCTURE_EDITING.md's own acceptance criteria §5/§9 — *"re-export reflects the edit"* — were **false on the branch.**

**Not introduced by Q3 — surfaced by it.** The defect was latent in the merged phase-2 core: the read path (Structure station) and the write path (EditBookUseCase) were wired, but the render path was not moved off the source bytes. It was found by **code inspection while preparing the front-matter Q3 commit** (re-verifying the export path against the actual code, the non-negotiable-#7 discipline), not by a user report and not by a real fixture failing. Naming it as a distinct defect — rather than folding the fix into the Q3 commit as an implementation detail — is a deliberate CTO decision (Option 2): a real defect in merged code deserves its own commit, its own proof, and its own reversal-grade record, so it can be audited or reverted without dragging the front-matter feature with it.

**The fix.** A shared render tail — `ExportManuscriptUseCase.renderBook(book, themeName, pageLayout)` and `PublishingUseCase.publishBook(book, themeName, pageLayout)` — that renders an already-built `Book`. `execute()` (the raw-bytes path, still used by the legacy `/api/manuscripts/export|publish` routes) parses then delegates to it; `ExportProjectUseCase`/`PublishProjectUseCase` pass the project's **stored** book (`ProjectService.currentBook(project)`). This also makes publish and export render the *same* book by construction — strengthening ADR-0045's original concern rather than reopening it. **Front-matter synthesis is unchanged in this commit** (still built in the render tail), so output is byte-for-byte identical for an unedited project; moving front matter to import time is the separate Q3 commit.

**This amends HOME_WORKSPACE.md Decision 6's phrase "export from the stored source."** "Stored source" now means the stored **book**, not the stored source **bytes**. The retained source asset is not discarded — it still earns its keep for a future higher-fidelity re-import (`AGGREGATES_AND_PERSISTENCE.md` Q5) — it is simply no longer what the renderer consumes.

**Proof (REAL_FIXTURE_POLICY).** `projectExportReflectsEdit.test.ts`: import the real 17-chapter corpus manuscript (`faith-alone-styled.docx`) through the app, confirm the baseline DOCX export has chapter 1's heading before the last chapter's, reorder the last chapter to the front through the real `POST /:id/structure` mutation route, and assert the re-exported `word/document.xml` now carries the moved chapter's heading first. A measured equality that fails for the right reason: if export ever regresses to re-parsing bytes, the order would not change and the test goes red.

**Lineage classification (mirrors the DROPCAP_TEXT_OVERLAP ruling).** **NOT counted in the eleven-bug real-fixture lineage.** That counter measures specifically what *real files reveal that synthetic coverage misses*; this defect was found by reading merged code, not by a real manuscript exposing it — the same category as `DROPCAP_TEXT_OVERLAP` (found by targeted instrumentation, CTO-excluded from the lineage). It is a real defect in merged code, closed with a real-fixture test, recorded here as its own category. See `docs/REAL_FIXTURE_POLICY.md`.

**Related:** STRUCTURE_EDITING.md (§5/§9 the criteria this restores; phase 2), ADR-0045 (publish/export must render the same book — now guaranteed structurally), ADR-0048/`PERSISTENCE.md` (the durable store the edited book lives in), `HOME_WORKSPACE.md` Decision 6 (the "stored source" phrase this amends), ADR-0001 (immutable Book), `TODO.md` → `STRUCTURE_EDITS_NOT_EXPORTED`.

---

## ADR-0053: `@dnd-kit` for Drag-and-Drop, and Why the Reorder Gesture Is Proven in a Real Browser, Not jsdom

**Status:** APPROVED (CTO 2026-07-21, `STRUCTURE_EDITING_PHASE3.md` D1 + Q5). Implemented in structure-editing Phase 3.

**The dependency.** Manual structure editing's frontend needs drag-to-reorder. Per `UI_FOUNDATION.md` Decision 1, this project builds UI in-house *except* where accessibility is genuinely hard and a headless library earns its place (the rule that already admitted Radix). Drag-and-drop with a working keyboard path and screen-reader announcements is exactly that case — decisive for the non-technical author audience and this project's 0-axe-node bar. **Chosen: `@dnd-kit`** (`core` + `sortable` + `utilities`): accessibility-first (built-in keyboard sensor + live-region announcements), zero runtime dependencies, tree-shakeable, React 19 / Next 16 compatible, and its `sortable` preset maps 1:1 onto "reorder a vertical list." **Rejected:** `react-beautiful-dnd` (archived, not React 19); `@hello-pangea/dnd` (maintained but heavier, less a11y-complete); native HTML5 DnD (touch + a11y burden).

**Spike-gated before the lock (ADR-0019/0020 precedent).** A throwaway spike confirmed, empirically in this exact React 19 / Next 16 install: installs with no peer conflict, compiles, renders, the KeyboardSensor activates a drag, collision detection runs, and the a11y announcement channel is live — zero console/React-19 errors. The dependency was locked only after that.

**The load-bearing finding — the reorder GESTURE cannot be driven in jsdom or a synthetic-event harness.** The spike (and later the real component) showed the drag *starts* everywhere but the move step never completes under jsdom (no layout engine drives dnd-kit's rAF/measuring/coordinate pipeline) or a headless synthetic-drag harness (no genuine OS pointer input for the PointerSensor). **This is environmental, not a dnd-kit or React-19 defect** — a real browser with real input drives it correctly (proven, below). The consequence for *how we test*, made a standing rule here:

> **A dnd-kit reorder's correctness is split for verification: the handler logic (our `onDragEnd` → indices → API call → server-authoritative apply) is unit-tested in jsdom, decoupled from the gesture; the gesture itself is proven only by a real-browser Playwright test.** Asserting the gesture in jsdom is not possible, and a test that *looks* like it exercises the drag but cannot actually move the item is worse than no test — it is a green-for-the-wrong-reason (the `TESTING_STRATEGY.md` honest-property principle).
